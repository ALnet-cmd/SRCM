    ...
    results.forEach(result => {
      if (standings[result.driver_id]) {
        ...
        if (result.position === 1) {
          standings[result.driver_id].wins++;
        }
        
        // Podi (posizioni 1-3)
        if (result.position <= 3) {
          standings[result.driver_id].podiums++;
        }
        
        // Giri veloci
        if (result.fastest_lap) {
          standings[result.driver_id].fastestLaps++;
        }
      }
    });
      ...
      {standings.length > 0 && (
        <div className="border-t p-4 bg-gray-50">
          <div className="flex justify-between items-center text-sm text-gray-600">
            <div>
              <strong>{t.totalDrivers}:</strong> {standings.length}
            </div>
            <div>
              <strong>{t.totalRaces}:</strong> {new Set(results.map(r => r.race_id)).size}
            </div>
            <div>
              <strong>{t.totalResults}:</strong> {results.length}
            </div>
          </div>
        </div>
      )}
